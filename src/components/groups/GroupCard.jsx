...
export default function GroupCard({ group, onEdit, onDelete }) {
  ...
  return (
    <Card className="h-100 shadow-sm border-0" style={{ borderTop: `4px solid ${cardColor}` }}>
      { isCreator && (
        <div className="position-absolute top-0 end-0 p-2 d-flex gap-1" style={{
          top:10,
          right:10,
          zIndex: 1,
        }}>
      ...
      <Card.Body className="d-flex flex-column p-3">
        ...
        <div className="d-flex gap-2 mb-3">
          ...
          <Badge bg="light" text="dark" className="border">
            📅 {group.createdAt ? new Date(group.createdAt).toLocaleDateString("en-IN", { month: "short", year: "numeric" }) : "Recently"}
          </Badge>
        </div>
